 ...
 *
 */
public class BluetoothConnActivity2 extends Activity implements OnClickListener {
...
//		stopService(new Intent(getApplicationContext(), BluetoothService.class));
	...
	public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
		...
		case BluetoothManager.REQUEST_FINE_LOCATION:
			...
			if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
				...
				// 获取权限失败
				Toast.makeText(BluetoothConnActivity2.this, "权限获取失败", 0).show();
	...
	private OnBTMeasureListener onBTMeasureListener = new OnBTMeasureListener() {
		...
		public void onMeasureResult(MeasurementResult result) {
			//测量结果
			Intent intent = new Intent(BluetoothConnActivity2.this, MeasurementResultActivity.class);
		...
		public void onMeasureError() {
			//测量错误
			Toast.makeText(BluetoothConnActivity2.this,
		...
		public void onFoundFinish(List<BluetoothDevice> deviceList) {
			...
			if(deviceList.size() == 0){
				Toast.makeText(BluetoothConnActivity2.this, "未搜索到设备", Toast.LENGTH_SHORT).show();
				finish();
			}
		}
